  ...
  for (const en of enrolled) {
    ...
    for (const step of campaign.steps) {
      try {
        ...
        if (errs.length) problems.push(`${c.empresa} ${step.id}: ${errs.map((e) => e.rule + " (" + e.detail + ")").join("; ")}`);
        else ok++;
      } catch (e) { problems.push(`${c.empresa} ${step.id}: render: ${e instanceof Error ? e.message : e}`); }
    }
  }
