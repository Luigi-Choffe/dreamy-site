...
async function main() {
  ...
  const [contacts, enrollments, companies] = await Promise.all([store.contacts(), store.enrollments(), store.companies()]);
...
<td style="padding:8px 12px;border-bottom:1px solid #e3e8e5;font-size:12px;color:#5c6660">${r.abertura}…</td>
  ...
  writeFileSync(process.argv[2]!, `<!doctype html><meta charset="utf-8"><title>Conexões LinkedIn — campanha construção</title>
