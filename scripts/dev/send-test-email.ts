...
try { process.loadEnvFile(".env.local"); } catch { /* env do shell */ }
const campaign = campaigns.find((c) => c.slug === "construcao-nova-receita")!;
const contact: Contact = {
  id: "teste", email: SIGNATURE.email, nome: "Luigi", empresa: "Incorporadora Exemplo",
  industria: campaign.industria, custom: { ...campaign.sampleCustom },
  importBatchId: "teste", verification: "ok", status: "active", createdAt: new Date().toISOString(),
};
async function main() {
  ...
  const ids = await client.sendBatch([{
    from, to: [SIGNATURE.email], subject: `[teste] ${b.subject}`,
    text: b.text, html: b.html, reply_to: SIGNATURE.email, headers: b.headers,
