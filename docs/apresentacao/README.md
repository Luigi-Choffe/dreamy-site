...
- **`dreamy-pitch.html`** — a versão "estado da arte" para apresentar: abra no browser (duplo clique; funciona offline, tudo embutido — fontes, logo). Animação constante (partículas, órbita com cometa, pacotes fluindo nos diagramas, curva que se desenha, discos respirando), entradas cinematográficas por slide e transições suaves. Controles: **← →** ou clique avança (Shift+clique volta, funciona com passador PgUp/PgDn), **F** tela cheia, **N** abre o roteiro do apresentador (o palco se reencaixa acima do painel), **Esc** fecha. Respeita `prefers-reduced-motion`. Shots de revisão: `pnpm tsx scripts/dev/pitch-shots.ts <pasta>`.
