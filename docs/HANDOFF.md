...
- Utilitários: `scripts/dev/ui-shots.ts <pasta> [base]` (banner de cookies, menu mobile, preferências, dropdown), `scripts/dev/form-shots.ts <pasta> [base]` (formulário: erros, passo 2, sucesso), `scripts/dev/deck-shots.ts <html> <pasta>` (screenshots por slide da apresentação), `scripts/dev/overflow-check.ts`, `reveal-check.ts`, `csp-check.ts`, `axe-check.ts` (dev), `pnpm qa:screenshots --viewports … --no-shots` (só checagens), `pnpm smoke --base <url>` (deploy).
