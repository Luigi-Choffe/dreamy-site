...
- Utilitários: `scripts/dev/ui-shots.ts <pasta> [base]` (banner de cookies, menu mobile, preferências, dropdown), `scripts/dev/overflow-check.ts`, `reveal-check.ts`, `csp-check.ts`, `axe-check.ts` (dev), `pnpm qa:screenshots --viewports … --no-shots` (só checagens), `pnpm smoke --base <url>` (deploy).
